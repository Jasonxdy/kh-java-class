...
public class BoardService {
	...
	/**
	 * 다중 조건 검색 중 총 게시글 수 조회용 Service
	 * 
	 * @param map
	 * @return listCount
	 * @throws Exception
	 */
	public int getSearchListCount2(Map<String, Object> map) throws Exception {
		SqlSession session = getSqlSession();

		int searchListCount = new BoardDao().getSearchListCount2(session, map);
	 ...
	 * @throws Exception
	 */
	public List<Board> selectSearchList2(Map<String, Object> map, PageInfo pInf) throws Exception {
		
		SqlSession session = getSqlSession();
		List<Board> searchList = new BoardDao().selectSearchList2(session, map, pInf);
		session.close();
		return searchList;
	}
